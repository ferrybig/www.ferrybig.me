...
export const tagAtom = route`/${'tag'}/atom.xml`({ lastOptional: true, tokenDoesMatchSlash: false }).component(AtomFeed);
export const period = route`/period/`().component(Debug);
export const byPeriod = route`/period/${'year'}-${'month'}/${'page'}`({ lastOptional: true }).component(Debug);
