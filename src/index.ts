...
import everyPost, { homePage, perPeriod, perTag } from './content';
...
export default function render(assets: Record<string, string>) {
	...
	const allRenderers: {
		...
		tag: perTag.flatMap(({tag, tagContent, content}) => content.map(
			(slice, page, { length: pages }) => base => renderRoute(base, routes.tag, {slice, page: page === 0 ? '' : page + 1, pages, tag, tagContent})
		)),
		tagAtom: perTag.map(({content, tag, tagContent}) => base => renderRoute(base, routes.tagAtom, {
			tag,
			slice: content[0],
